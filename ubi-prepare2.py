...
# Copyright (C) 2014 NFSCHINA.
# Written by Li Ting <liting@nfschina.com>
#
...
class PageGtk(plugin.PluginUI):
    ...
    def __init__(self, controller, *args, **kwargs):
        ...
        builder.add_from_file(os.path.join(
            os.environ['UBIQUITY_GLADE'], 'stepPrepare2.ui'))
        ...
        self.partitioninfo = builder.get_object(
            'partitioninfo')
        self.modelcode = builder.get_object(
            'modelcode')
        self.layoutcode = builder.get_object(
            'layoutcode')
...
# add modelcode and layoutcode,these two are not visible to user
# just used for getting text, for keyboardmodel and keyboardlayout are not plain text
    def set_modelcode_info(self,modelcode):
        if modelcode:
            self.modelcode.set_text(modelcode)

    def get_modelcode_info(self):
        return self.modelcode.get_text()

    def set_layoutcode_info(self, layoutcode):
        if layoutcode:
            self.layoutcode.set_text(layoutcode)

    def get_layoutcode_info(self):
...
#        user_sure = self.db.get('ubiquity/user_sure') == 'true'
        ...
        if langinfo == 'zh_CN':
            ...
            language = "English"
        self.ui.set_modelcode_info(keyboardmodel)
        self.ui.set_layoutcode_info(keyboardlayout)
 # get keyboardmodelcode and keyboard layoutcode
        modelcodetext=self.ui.get_modelcode_info()
        layoutcodetext=self.ui.get_layoutcode_info()

        if modelcodetext == 'pc105' and layoutcodetext == 'cn':
            tkeyboards = "汉语键盘"
        elif layoutcodetext == 'us':
            tkeyboards = "英语键盘"
        else:
            tkeyboards = "其他键盘"       
        ...
        self.ui.set_keyboard_info(tkeyboards)
